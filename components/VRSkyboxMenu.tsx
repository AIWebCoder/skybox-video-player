    ...
    <div ref={sceneRef} style={{ width: '100%', height: '100vh' }}>
      ...
      >
        <a-assets>
          ...
          {channels.map((ch) => (
            <img key={ch.id} id={`icon-${ch.id}`} src={ch.icon} crossOrigin="anonymous" />
          ))}
          {videos.map((v) => (
        ...
        >
          ...
          {(activeTab === 'favorites' ? favorites : history).slice(0, 6).map((item, index) => (
            <a-entity
              key={item.id}
            ...
            >
              <a-image
                src={`#${activeTab === 'favorites' ? 'fav-thumb' : 'hist-thumb'}-${item.id}`}
                width="0.25"
                height="0.15"
